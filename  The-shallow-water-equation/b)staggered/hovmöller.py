...
# Load data
# ----------------------------
h_analytic = np.loadtxt("b)staggered/h-analytic.txt")
h_leapfrog = np.loadtxt("b)staggered/h-leapfrog-.0250.txt")

u_analytic = np.loadtxt("b)staggered/u-analytic.txt")
...
plt.suptitle(r"Staggered grid ($\mu = 0.45$, $\Delta x = 0.025$)", fontsize=15)
...
plt.savefig(f"{output_dir}/staggered-hovmoller.png", dpi=300)
plt.show()