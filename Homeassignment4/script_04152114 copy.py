...
# -----------------------------
# PLOT (h(x) at selected times)
# -----------------------------
fig, axes = plt.subplots(1, 4, figsize=(14, 3), sharey=True)

# times you want to visualize (as fraction of total simulation)
time_fractions = [0.0, 0.25, 0.5, 0.9]
...
for ax, mu in zip(axes, mus):
    ...
    indices = [int(f * (Nt-1)) for f in time_fractions]

    for idx in indices:
        ax.plot(x, H_store[idx, :], label=f"t={idx*dt:.2f}")

    ax.set_title(fr'$\mu={mu}$')
    ax.set_xlabel('x')
    ax.grid()

axes[0].set_ylabel('h(x,t)')

# one legend (optional: move outside)
